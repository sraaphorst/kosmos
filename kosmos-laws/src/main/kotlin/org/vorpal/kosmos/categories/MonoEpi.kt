...
object MonoEpi {
    fun <X: Any, A: Any, B: Any> monoLawHolds(
        f: Morphism<A, B>,
        setX: FiniteSet<X>,
        setA: FiniteSet<A>,
        eqA: (A, A) -> Boolean,
        eqB: (B, B) -> Boolean,
        samples: Sequence<Pair<Morphism<X, A>, Morphism<X, A>>>
    ): Boolean {
        val xs = setX.toList()
    ...
    fun <A: Any, B: Any, Y: Any> epiLawHolds(
        f: Morphism<A, B>,
        setA: FiniteSet<A>,
        setB: FiniteSet<B>,
        setY: FiniteSet<Y>,
        eqB: (B, B) -> Boolean,
        eqY: (Y, Y) -> Boolean,
        samples: Sequence<Pair<Morphism<B, Y>, Morphism<B, Y>>>
    ): Boolean {
