...
class ScalarFieldSpec : FunSpec({
    ...
    context("ScalarField evaluation") {
        ...
        test("constant field returns same value everywhere") {
            checkAll(arbFieldReal(), arbVec2Real()) { value, point ->
                val field = ScalarField.constant(vec2Space, value)
        ...
        test("zero field returns additive identity everywhere") {
            checkAll(arbVec2Real()) { point ->
                val field = ScalarField.zero(vec2Space)
        ...
        test("one field returns multiplicative identity everywhere") {
            checkAll(arbVec2Real()) { point ->
                val field = ScalarField.one(vec2Space)
        ...
        test("of() creates field that evaluates correctly") {
            checkAll(arbVec2Real()) { point ->
                val field = ScalarField.of(vec2Space) { v -> v.x * 2.0 + v.y * 3.0 }
    ...
    context("Pointwise addition (+)") {
        ...
        test("zero is additive identity: f + 0 = f") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                val zero = ScalarField.zero(f.space)
                val result = f + zero
    ...
    context("Pointwise multiplication (*)") {
        ...
        test("multiplication by zero gives zero: f * 0 = 0") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                val zero = ScalarField.zero(vec2Space)
                val result = f * zero
    ...
    context("Pointwise division (/)") {
        ...
        test("division by one is identity: f / 1 = f") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                val one = ScalarField.one(f.space)
                val result = f / one
                result(point) shouldBeApproximately f(point)
            }
        }
        ...
        test("self-division gives one (when non-zero): f / f = 1") {
            checkAll(arbNonZeroScalarField(), arbVec2Real()) { f, point ->
                val result = f / f
    ...
    context("Scalar multiplication") {
        ...
        test("scalar multiplication from right: f * c") {
            checkAll(arbScalarField(), arbFieldReal(), arbVec2Real()) { f, c, point ->
                val result = f * c
        ...
        test("scalar multiplication from left: c * f") {
            checkAll(arbFieldReal(), arbScalarField(), arbVec2Real()) { c, f, point ->
                val result = c * f
    ...
    context("andThen transformation") {
        ...
        test("andThen applies function pointwise") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                val realDoubler = f.andThen { 2.0 * it }
                realDoubler(point) shouldBeApproximately (f(point) * 2.0)
            }
        }

        test("andThen with identity is identity") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                val mapped = Identity<Real>().compose(f)
        ...
        test("andThen composition") {
            checkAll(arbScalarField(), arbVec2Real()) { f, point ->
                ...
                val right = f.andThen { h(g(it)) }

                left(point) shouldBeApproximately right(point)
            }
        }

        test("andThen preserves field structure through linear transformations") {
            checkAll(
                arbScalarField(),
                arbScalarField(),
                arbFieldReal(),
                ...
                val mapped = (f + g).andThen(linear)
                val separate = f.andThen(linear) + g.andThen(linear)
    ...
    context("Functional composition (∘)") {
        ...
        test("compose creates correct composite function") {
            checkAll(arbVec2Real()) { point ->
                val f = ScalarField.of(vec2Space) { v -> v.x + v.y }
    ...
    context("Combined operations") {
        ...
        test("rational expression: (f + g) / (h + 1)") {
            checkAll(
                ...
                val inv = h(point) + 1.0
                if (inv != 0.0) {
                    val expected = (f(point) + g(point)) / inv
                    result(point) shouldBeApproximately expected
                }
            }
        }

        test("field with scalar and andThen: (c * f).andThen { it + d }") {
            checkAll(
                arbFieldReal(),
                arbFieldReal(),
                arbScalarField(),
    ...
    context("Edge cases") {
        ...
        test("operations on constant fields") {
            checkAll(arbFieldReal(), arbFieldReal(), arbVec2Real()) { a, b, point ->
                val fieldA = ScalarField.constant(vec2Space, a)
                val fieldB = ScalarField.constant(vec2Space, b)
