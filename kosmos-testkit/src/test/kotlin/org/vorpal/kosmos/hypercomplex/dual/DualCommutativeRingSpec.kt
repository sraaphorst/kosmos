...
/**
 * Property tests for [DualAlgebras.DualCommutativeRingWithReciprocal] over the reals.
 *
 * The structural ring axioms (Abelian group on +, commutative monoid on *,
 * bilateral distributivity, identities) are delegated to [CommutativeRingLaws].
 *
 * The lift R → Dual<R> is a unital ring homomorphism, exercised via
 * [preservesBinaryOpLaw] and [preservesIdentityLaw].
 *
 * The remaining tests cover dual-specific behavior:
 *  - the carrier (construction, lift, eps, epsOne);
 *  - explicit multiplication / subtraction / reciprocal formulas;
 *  - epsilon nilpotency (ε² = 0);
 *  - the partial reciprocal and its characterization
 *    (recall: dual rings with reciprocal are *not* fields — pure infinitesimals
 *    are non-invertible zero divisors);
 *  - division;
 *  - automatic differentiation;
 *  - edge cases and string rendering.
 */
class DualCommutativeRingSpec : FunSpec({

    val dualRing = RealAlgebras.RealField.dualRingWithReciprocal {
        RealAlgebras.eqRealApprox(it, 0.0)
    }

    // Convenience values and operators.
    ...
    val prDualCompact = DualAlgebras.printableDualRealCompactSigned
    ...
    //
    // CommutativeRingLaws covers everything the original spec checked under
    // "Addition - Abelian Group", "Multiplication - Monoid", and
    // "Ring distributivity": associativity, commutativity, identities,
    // additive inverses, bilateral distributivity, and annihilation
    // (the latter being entailed by distributivity + the abelian group on +).

    context("Commutative ring laws") {
    ...
    // ── lift : R → Dual<R> as a unital ring homomorphism ───────────────────
    //
    // The structural homomorphism axioms are checked via the homomorphism
    // law builders. The "commutes with ε" test stays bespoke, because it is
    // a property specific to dual numbers, not a generic homomorphism law.
    ...
    context("Lift is a unital ring homomorphism") {
        ...
        test("lift preserves addition: lift(a + b) = lift(a) + lift(b)") {
            preservesBinaryOpLaw(
                domainOp = baseField.add.op,
                codomainOp = dualRing.add.op,
                hom = dualRing::lift,
                arbA = baseArb,
                eqB = eqDual,
                prA = prReal,
                prB = prDual,
                label = "lift preserves +"
            ).test()
        }

        test("lift preserves multiplication: lift(a * b) = lift(a) * lift(b)") {
            preservesBinaryOpLaw(
                domainOp = baseField.mul.op,
                codomainOp = dualRing.mul.op,
                hom = dualRing::lift,
                arbA = baseArb,
                eqB = eqDual,
                prA = prReal,
        ...
        test("lift preserves the multiplicative identity: lift(1) = 1 + 0ε") {
            preservesIdentityLaw(
                domainIdentity = baseField.one,
                codomainIdentity = dualRing.mul.identity,
                hom = dualRing::lift,
                eqB = eqDual,
                prA = prReal,
                prB = prDual,
                label = "lift preserves 1"
            ).test()
        }
    ...
    context("String representations") {
        ...
        test("Printable format, positive") {
            val d = dual(3.0, 4.0)
            prDual(d) shouldBe "3.0 + 4.0$eps"
            prDualCompact(d) shouldBe "3.0 + 4.0$eps"
        }
        ...
        test("zero dual toString") {
            prDual(zero) shouldBe "0.0 + 0.0$eps"
            prDualCompact(zero) shouldBe "0.0"
        }

        test("one dual toString") {
            prDual(one) shouldBe "1.0 + 0.0ε"
            prDualCompact(one) shouldBe "1.0"
        }

        test("epsilon toString") {
            val e = dualRing.epsOne
            prDual(e) shouldBe "0.0 + 1.0ε"
            prDualCompact(e) shouldBe eps
        }
    }
})
