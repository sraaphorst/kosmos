...
/**
 * These define the maps from one step below the quaternions to the embeddings into the
 * quaternion families. For example:
 ...
 *
 * 2. The Eisenstein are not defined by where i goes, but by where ω goes, with:
 * ```kotlin
 * ω^2 + ω + 1 = 0
 * ```
 ...
 * that already contains an element s with s^2 = -3 (a “√-3”), and realize ω via (-1 + s)/2.
 *
 * If your coefficient ring is Z[ω], then building a quaternion algebra over it is different:
