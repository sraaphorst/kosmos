...
/**
 * Join-semilattice (A, ∨). Laws: associative, commutative, idempotent.
...
/**
 * Meet-semilattice (A, ∧). Laws: associative, commutative, idempotent.
...
object JoinSemilattices {
    fun <A : Any> of(join: BinOp<A>): JoinSemilattice<A> =
        object : JoinSemilattice<A> { override val join = join }
}

object MeetSemilattices {
    fun <A : Any> of(meet: BinOp<A>): MeetSemilattice<A> =
        object : MeetSemilattice<A> { override val meet = meet }
}
