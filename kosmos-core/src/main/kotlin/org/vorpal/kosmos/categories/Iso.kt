...
object Isos {
    ...
    fun <A> identity(): Iso<A, A> = Iso({ it }, { it })

    /**
     * If `f` is bijective on these finite sets (w.r.t. eqA/eqB), construct its inverse and return an Isomorphism.
     * Returns null if `f` is not bijective over (domain, codomain).
     */
    fun <A, B> isoFromBijection(
        f: Morphism<A, B>,
        domain: FiniteSet<A>,
