...
class BijectionSpec : FunSpec({
    ...
    val setA = FiniteSet.ordered(1, 2, 3)
    val setB = FiniteSet.ordered("a", "b", "c")
    val setC = FiniteSet.ordered('x', 'y', 'z')

    context("Bijection construction") {
        test("should create valid bijection from complete forward map") {
            val bij = Bijection.of(
                setA, setB,
        ...
        test("should create valid bijection using build DSL") {
            val bij = Bijection.build(setA, setB) {
                this[1] = "a"
                this[2] = "b"
                this[3] = "c"
            }
        ...
        test("should fail when forward map is not injective") {
            val setSmall = FiniteSet.ordered("a", "b")
            shouldThrow<IllegalArgumentException> {
                Bijection.of(
                    setA, setSmall,
    ...
    context("Bijection backward (inverse) mapping") {
        ...
        test("property: forward then backward is identity") {
            checkAll(100, generateArbPermutation(Arb.int(1..100), 2, 15)) { bij ->
                bij.domain.all { a ->
    ...
    context("Bijection with singleton and empty sets") {
        test("should work with singleton sets") {
            val single1 = FiniteSet.singleton(42)
            val single2 = FiniteSet.singleton("answer")
        ...
        test("should work with empty sets") {
            val empty1 = FiniteSet.empty<Int>()
            val empty2 = FiniteSet.empty<String>()

            val bij = Bijection.of(empty1, empty2, emptyMap())
    ...
    context("Endomorphic bijections (permutations)") {
        ...
        test("property: permutation composition preserves bijection properties") {
            checkAll(50, BijectionTestingCombinations.arbPermutationPair(Arb.int(1..100), 6)) { (perm1, perm2) ->
    ...
    context("Bijection inverse operation") {
        ...
        test("property: double inverse returns to original") {
            checkAll(80, generateArbPermutation(Arb.int(1..100), 2, 12)) { bij ->
    ...
    context("Bijection composition") {
        test("should compose two bijections via then") {
            val bij1 = Bijection.of(
                setA, setB,
                mapOf(1 to "a", 2 to "b", 3 to "c")
            )

            val bij2 = Bijection.of(
                setB, setC,
        ...
        test("property: associativity of composition") {
            checkAll(40, generateArbOrderedFiniteSetOfSize(Arb.int(1..100), 5)) { domain ->
                checkAll(10, Arb.shuffle(domain.order)) { perm1 ->
                    checkAll(10, Arb.shuffle(domain.order)) { perm2 ->
                        checkAll(10, Arb.shuffle(domain.order)) { perm3 ->
                            val f = Bijection.endo(domain, domain.order.zip(perm1).toMap())
    ...
    context("Bijection equality on domain") {
        test("eqOn should detect equal bijections") {
            val bij1 = Bijection.of(setA, setB, mapOf(1 to "a", 2 to "b", 3 to "c"))
            val bij2 = Bijection.of(setA, setB, mapOf(1 to "a", 2 to "b", 3 to "c"))
        ...
        test("eqOn should detect different bijections") {
            val bij1 = Bijection.of(setA, setB, mapOf(1 to "a", 2 to "b", 3 to "c"))
            val bij2 = Bijection.of(setA, setB, mapOf(1 to "b", 2 to "a", 3 to "c"))
    ...
    context("Orbit and permutation operations") {
        test("orbit should find cycle") {
            val bij = Bijection.endo(setA, mapOf(1 to 2, 2 to 3, 3 to 1))
        ...
        test("property: orbit is closed under application") {
            checkAll(80, BijectionTestingCombinations.arbBijectionWithElement(Arb.int(1..100), 2, 10)) { (bij, elem) ->
        ...
        test("property: cyclic permutations have single orbit") {
            checkAll(60, generateArbCyclicPermutation(Arb.int(1..100), 3, 10)) { bij ->
        ...
        test("property: involutions have order 1 or 2") {
            checkAll(60, generateArbInvolution(Arb.int(1..100), 2, 12)) { bij ->
                bij.domain.all { elem ->
                    val order = bij.orderOf(elem)
                    order == 1 || order == 2
                } shouldBe true
        ...
        test("property: cycle decomposition covers entire domain") {
            checkAll(60, generateArbPermutation(Arb.int(1..100), 3, 10)) { bij ->
    ...
    context("Automorphism orbit operations") {
        ...
        test("property: automorphism orbits are closed") {
            checkAll(60, generateArbPermutation(Arb.int(1..100), 2, 10)) { bij ->
                val auto = bij.toAutomorphism()
        ...
        test("automorphism isIdentity should detect identity") {
            val identity = Automorphism.id<Int>()
            val notIdentity = Automorphism.of<Int>(
        ...
        test("property: identity automorphism on any domain") {
            checkAll(60, generateArbOrderedFiniteSet(Arb.int(1..100), 2, 12)) { domain ->
                val identity = Automorphism.id<Int>()
    ...
    context("Special bijection factories") {
        test("fromOrdering should create position-based bijection") {
            val domain = FiniteSet.ordered(1, 2, 3)
            val codomain = FiniteSet.ordered("x", "y", "z")

            val bij = Bijection.fromOrdering(domain, codomain)
        ...
        test("fromCycles should create valid permutation") {
            val domain = FiniteSet.ordered(1, 2, 3, 4, 5)
            val bij = Bijection.fromCycles(domain, listOf(1, 2, 3), listOf(4, 5))
        ...
        test("property: fromCycles produces correct cycle structure") {
            checkAll(60, generateArbOrderedFiniteSet(Arb.int(1..100), 5, 10)) { domain ->
                val cycle1 = domain.take(3).order
                val cycle2 = domain.drop(3).take(2).order

                if (cycle1.isNotEmpty() && cycle2.isNotEmpty()) {
                    val bij = Bijection.fromCycles(domain, cycle1, cycle2)
