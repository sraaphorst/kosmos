...
fun <A> Eq<A>.nullable(): Eq<A?> =
    Eq { x, y -> if (x === y) true else x != null && y != null && this.eqv(x, y) }
...
/** Unordered set equality using this Eq for membership; O(n^2). */
...
object Eqs {
    ...
    // Time (choose what you need)
    val instant: Eq<Instant> = Eq { a, b -> a == b }
    val localDate: Eq<LocalDate> = Eq { a, b -> a == b }
    val localTime: Eq<LocalTime> = Eq { a, b -> a == b }
    val localDateTime: Eq<LocalDateTime> = Eq { a, b -> a == b }
    val offsetDateTime: Eq<OffsetDateTime> = Eq { a, b -> a == b }
    val zonedDateTime: Eq<ZonedDateTime> = Eq { a, b -> a == b }
    val duration: Eq<Duration> = Eq { a, b -> a == b }
